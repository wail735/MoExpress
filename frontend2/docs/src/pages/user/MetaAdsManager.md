# 📚 Documentation Technique Détaillée : `MetaAdsManager.jsx`

**Emplacement Source** : `src/pages/user/MetaAdsManager.jsx`  
**Portée** : Frontend Client React (Vite SPA)  
**Nombre de Lignes Code** : 210 lignes  

---

## 1. 📌 Rôle et Responsabilité du Fichier

Le fichier `MetaAdsManager.jsx` est un composant clé de l'architecture MoExpress (AliExpress Clone).
...
| L7 | `import { Megaphone, Upload, DollarSign, Calendar, Layout, Eye, MousePointer, Play, CheckCircle } from "lucide-react";` | Module indispensable pour les fonctionnalités du fichier. |
...
#### 🔹 `MetaAdsManager` (Ligne 11)
- **Signature / Extrait** : `export const MetaAdsManager = () => {`
...
#### 🔹 `totalCost` (Ligne 31)
- **Signature / Extrait** : `const totalCost = (rates[placement] \|\| 10) * durationDays;`
- **Rôle Fonctionnel** : Cette fonction/méthode est responsable de l'exécution d'une étape clé du traitement.
- **Mode d'Exécution** : Synchrone.

#### 🔹 `handleLaunchCampaign` (Ligne 46)
- **Signature / Extrait** : `const handleLaunchCampaign = (e) => {`
- **Rôle Fonctionnel** : Cette fonction/méthode est responsable de l'exécution d'une étape clé du traitement.
- **Mode d'Exécution** : Synchrone.
