# 📚 Documentation Technique Détaillée : `Login.jsx`

**Emplacement Source** : `src/pages/auth/Login.jsx`  
...
Le fichier `Login.jsx` est un composant clé de l'architecture MoExpress (AliExpress Clone).
...
| L8 | `import { ShoppingBag, Mail, Lock, LogIn, ArrowRight } from "lucide-react";` | Module indispensable pour les fonctionnalités du fichier. |
...
#### 🔹 `Login` (Ligne 13)
- **Signature / Extrait** : `export const Login = () => {`
...
#### 🔹 `handleGoogleLogin` (Ligne 22)
- **Signature / Extrait** : `const handleGoogleLogin = async () => {`
- **Rôle Fonctionnel** : Cette fonction/méthode est responsable de l'exécution d'une étape clé du traitement.
- **Mode d'Exécution** : Asynchrone avec retour de Promesse (`Promise`).

#### 🔹 `handleSubmit` (Ligne 35)
