# 📚 Documentation Technique Détaillée : `FranchiseDirectory.jsx`

**Emplacement Source** : `src/pages/public/FranchiseDirectory.jsx`  
**Portée** : Frontend Client React (Vite SPA)  
**Nombre de Lignes Code** : 163 lignes  

---

## 1. 📌 Rôle et Responsabilité du Fichier

Le fichier `FranchiseDirectory.jsx` est un composant clé de l'architecture MoExpress (AliExpress Clone).
...
| L7 | `import { Building, MapPin, Phone, ExternalLink, Search, Plus, X } from "lucide-react";` | Module indispensable pour les fonctionnalités du fichier. |
...
#### 🔹 `FranchiseDirectory` (Ligne 10)
- **Signature / Extrait** : `export const FranchiseDirectory = () => {`
...
#### 🔹 `handleApply` (Ligne 29)
- **Signature / Extrait** : `const handleApply = (e) => {`
- **Rôle Fonctionnel** : Cette fonction/méthode est responsable de l'exécution d'une étape clé du traitement.
- **Mode d'Exécution** : Synchrone.

#### 🔹 `handleDirections` (Ligne 40)
- **Signature / Extrait** : `const handleDirections = (fName) => {`
- **Rôle Fonctionnel** : Cette fonction/méthode est responsable de l'exécution d'une étape clé du traitement.
- **Mode d'Exécution** : Synchrone.
