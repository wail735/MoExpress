# 📚 Documentation Technique Détaillée : `apiClient.js`

**Emplacement Source** : `src/api/apiClient.js`  
...
Le fichier `apiClient.js` est un composant clé de l'architecture MoExpress (AliExpress Clone).
...
| :--- | :--- | :--- |
| L1 | `import axios from 'axios'` | Module indispensable pour les fonctionnalités du fichier. |
...
#### 🔹 `apiClient` (Ligne 5)
- **Signature / Extrait** : `export const apiClient = axios.create({`
...
- **`Axios / apiClient`** : Client HTTP basé sur les Promesses pour émettre des requêtes REST sérialisées en JSON vers le serveur backend.
