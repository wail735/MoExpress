# 📚 Documentation Technique Détaillée : `NotificationContext.jsx`

**Emplacement Source** : `src/context/NotificationContext.jsx`  
**Portée** : Frontend Client React (Vite SPA)  
**Nombre de Lignes Code** : 56 lignes  

---

## 1. 📌 Rôle et Responsabilité du Fichier

Le fichier `NotificationContext.jsx` est un composant clé de l'architecture MoExpress (AliExpress Clone).
...
| :--- | :--- | :--- |
| L6 | `import React, { createContext, useContext, useState } from "react";` | Module indispensable pour les fonctionnalités du fichier. |
...
#### 🔹 `NotificationProvider` (Ligne 10)
- **Signature / Extrait** : `export const NotificationProvider = ({ children }) => {`
...
#### 🔹 `addToast` (Ligne 13)
- **Signature / Extrait** : `const addToast = (message, type = "info", duration = 4000) => {`
- **Rôle Fonctionnel** : Cette fonction/méthode est responsable de l'exécution d'une étape clé du traitement.
- **Mode d'Exécution** : Synchrone.

#### 🔹 `removeToast` (Ligne 22)
- **Signature / Extrait** : `const removeToast = (id) => {`
- **Rôle Fonctionnel** : Cette fonction/méthode est responsable de l'exécution d'une étape clé du traitement.
- **Mode d'Exécution** : Synchrone.

#### 🔹 `useNotification` (Ligne 55)
- **Signature / Extrait** : `export const useNotification = () => useContext(NotificationContext);`
- **Rôle Fonctionnel** : Cette fonction/méthode est responsable de l'exécution d'une étape clé du traitement.
- **Mode d'Exécution** : Synchrone.
