# 📚 Documentation Technique Détaillée : `ThemeContext.jsx`

**Emplacement Source** : `src/context/ThemeContext.jsx`  
...
Le fichier `ThemeContext.jsx` est un composant clé de l'architecture MoExpress (AliExpress Clone).
...
#### 🔹 `ThemeProvider` (Ligne 10)
- **Signature / Extrait** : `export const ThemeProvider = ({ children }) => {`
...
#### 🔹 `toggleTheme` (Ligne 25)
- **Signature / Extrait** : `const toggleTheme = () => setIsDark((prev) => !prev);`
- **Rôle Fonctionnel** : Cette fonction/méthode est responsable de l'exécution d'une étape clé du traitement.
- **Mode d'Exécution** : Synchrone.

#### 🔹 `useTheme` (Ligne 34)
- **Signature / Extrait** : `export const useTheme = () => useContext(ThemeContext);`
- **Rôle Fonctionnel** : Cette fonction/méthode est responsable de l'exécution d'une étape clé du traitement.
- **Mode d'Exécution** : Synchrone.
