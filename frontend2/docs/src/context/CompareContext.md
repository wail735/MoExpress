# 📚 Documentation Technique Détaillée : `CompareContext.jsx`

**Emplacement Source** : `src/context/CompareContext.jsx`  
...
Le fichier `CompareContext.jsx` est un composant clé de l'architecture MoExpress (AliExpress Clone).
...
#### 🔹 `CompareProvider` (Ligne 10)
- **Signature / Extrait** : `export const CompareProvider = ({ children }) => {`
...
#### 🔹 `addToCompare` (Ligne 13)
- **Signature / Extrait** : `const addToCompare = (product) => {`
- **Rôle Fonctionnel** : Cette fonction/méthode est responsable de l'exécution d'une étape clé du traitement.
- **Mode d'Exécution** : Synchrone.

#### 🔹 `removeFromCompare` (Ligne 22)
- **Signature / Extrait** : `const removeFromCompare = (id) => {`
- **Rôle Fonctionnel** : Cette fonction/méthode est responsable de l'exécution d'une étape clé du traitement.
- **Mode d'Exécution** : Synchrone.

#### 🔹 `clearCompare` (Ligne 26)
- **Signature / Extrait** : `const clearCompare = () => setCompareItems([]);`
- **Rôle Fonctionnel** : Cette fonction/méthode est responsable de l'exécution d'une étape clé du traitement.
- **Mode d'Exécution** : Synchrone.

#### 🔹 `useCompare` (Ligne 35)
- **Signature / Extrait** : `export const useCompare = () => useContext(CompareContext);`
- **Rôle Fonctionnel** : Cette fonction/méthode est responsable de l'exécution d'une étape clé du traitement.
- **Mode d'Exécution** : Synchrone.
