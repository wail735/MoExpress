# 📚 Documentation Technique Détaillée : `support.service.js`

**Emplacement Source** : `support/support.service.js`  
**Portée** : Backend Serveur (Node.js / Express)  
**Nombre de Lignes Code** : 70 lignes  

---

## 1. 📌 Rôle et Responsabilité du Fichier

Le fichier `support.service.js` est un composant clé de l'architecture MoExpress (AliExpress Clone).
...
| :--- | :--- | :--- |
| L6 | `import SupportTicket from "./support.model.js";` | Module indispensable pour les fonctionnalités du fichier. |
...
#### 🔹 `createTicket` (Ligne 13)
- **Signature / Extrait** : `export const createTicket = async (userId, { subject, message, priority }) => {`
...
#### 🔹 `replyTicket` (Ligne 36)
- **Signature / Extrait** : `export const replyTicket = async (ticketId, { response, status }) => {`
- **Rôle Fonctionnel** : Cette fonction/méthode est responsable de l'exécution d'une étape clé du traitement.
- **Mode d'Exécution** : Asynchrone avec retour de Promesse (`Promise`).

#### 🔹 `getUserTickets` (Ligne 61)
- **Signature / Extrait** : `export const getUserTickets = async (userId) => {`
- **Rôle Fonctionnel** : Cette fonction/méthode est responsable de l'exécution d'une étape clé du traitement.
- **Mode d'Exécution** : Asynchrone avec retour de Promesse (`Promise`).
