# 📚 Documentation Technique Détaillée : `newsletter.service.js`

**Emplacement Source** : `newsletter/newsletter.service.js`  
**Portée** : Backend Serveur (Node.js / Express)  
**Nombre de Lignes Code** : 124 lignes  

---

## 1. 📌 Rôle et Responsabilité du Fichier

Le fichier `newsletter.service.js` est un composant clé de l'architecture MoExpress (AliExpress Clone).
...
| :--- | :--- | :--- |
| L6 | `import Newsletter from "./newsletter.model.js";` | Module indispensable pour les fonctionnalités du fichier. |
| L7 | `import { sendEmail } from "../config/email.js";` | Module indispensable pour les fonctionnalités du fichier. |
...
#### 🔹 `subscribeNewsletter` (Ligne 12)
- **Signature / Extrait** : `export const subscribeNewsletter = async (email) => {`
...
#### 🔹 `unsubscribeNewsletter` (Ligne 52)
- **Signature / Extrait** : `export const unsubscribeNewsletter = async (email) => {`
- **Rôle Fonctionnel** : Cette fonction/méthode est responsable de l'exécution d'une étape clé du traitement.
- **Mode d'Exécution** : Asynchrone avec retour de Promesse (`Promise`).

#### 🔹 `getAllSubscribers` (Ligne 70)
- **Signature / Extrait** : `export const getAllSubscribers = async (page = 1, limit = 50) => {`
- **Rôle Fonctionnel** : Cette fonction/méthode est responsable de l'exécution d'une étape clé du traitement.
- **Mode d'Exécution** : Asynchrone avec retour de Promesse (`Promise`).
...
#### 🔹 `sendNewsletterCampaign` (Ligne 83)
- **Signature / Extrait** : `export const sendNewsletterCampaign = async ({ subject, htmlContent }) => {`
- **Rôle Fonctionnel** : Cette fonction/méthode est responsable de l'exécution d'une étape clé du traitement.
- **Mode d'Exécution** : Asynchrone avec retour de Promesse (`Promise`).
