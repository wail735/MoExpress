# 📚 Documentation Technique Détaillée : `email.js`

**Emplacement Source** : `config/email.js`  
...
Le fichier `email.js` est un composant clé de l'architecture MoExpress (AliExpress Clone).
...
| :--- | :--- | :--- |
| L7 | `import nodemailer from "nodemailer";` | Module indispensable pour les fonctionnalités du fichier. |
...
#### 🔹 `createTransporter` (Ligne 13)
- **Signature / Extrait** : `const createTransporter = () => {`
...
#### 🔹 `sendEmail` (Ligne 43)
- **Signature / Extrait** : `export const sendEmail = async (options) => {`
- **Rôle Fonctionnel** : Cette fonction/méthode est responsable de l'exécution d'une étape clé du traitement.
- **Mode d'Exécution** : Asynchrone avec retour de Promesse (`Promise`).

#### 🔹 `sendPasswordResetEmail` (Ligne 72)
- **Signature / Extrait** : `export const sendPasswordResetEmail = async (userEmail, resetToken) => {`
- **Rôle Fonctionnel** : Cette fonction/méthode est responsable de l'exécution d'une étape clé du traitement.
- **Mode d'Exécution** : Asynchrone avec retour de Promesse (`Promise`).
...
- **`Nodemailer (SMTP SSL/TLS Port 465)`** : Transporteur de messagerie sécurisé pour la transmission réelle d'emails transactionnels.
