# 📚 Documentation Technique Détaillée : `cloudinary.js`

**Emplacement Source** : `config/cloudinary.js`  
...
Le fichier `cloudinary.js` est un composant clé de l'architecture MoExpress (AliExpress Clone).
...
| :--- | :--- | :--- |
| L7 | `import { v2 as cloudinary } from "cloudinary";` | Module indispensable pour les fonctionnalités du fichier. |
| L10 | `import fs from "fs";` | Module indispensable pour les fonctionnalités du fichier. |
...
#### 🔹 `uploadImage` (Ligne 31)
- **Signature / Extrait** : `export const uploadImage = async (filePath, options = {}) => {`
...
#### 🔹 `deleteImage` (Ligne 75)
- **Signature / Extrait** : `export const deleteImage = async (publicId) => {`
- **Rôle Fonctionnel** : Cette fonction/méthode est responsable de l'exécution d'une étape clé du traitement.
- **Mode d'Exécution** : Asynchrone avec retour de Promesse (`Promise`).
