# 📚 Documentation Technique Détaillée : `redis.js`

**Emplacement Source** : `config/redis.js`  
**Portée** : Backend Serveur (Node.js / Express)  
**Nombre de Lignes Code** : 135 lignes  

---

## 1. 📌 Rôle et Responsabilité du Fichier

Le fichier `redis.js` est un composant clé de l'architecture MoExpress (AliExpress Clone).
...
| :--- | :--- | :--- |
| L7 | `import redis from "redis";` | Module indispensable pour les fonctionnalités du fichier. |
...
#### 🔹 `createRedisClient` (Ligne 24)
- **Signature / Extrait** : `export const createRedisClient = () => {`
...
#### 🔹 `cacheData` (Ligne 77)
- **Signature / Extrait** : `export const cacheData = async (client, key, data, ttl = 3600) => {`
- **Rôle Fonctionnel** : Cette fonction/méthode est responsable de l'exécution d'une étape clé du traitement.
- **Mode d'Exécution** : Asynchrone avec retour de Promesse (`Promise`).

#### 🔹 `getCachedData` (Ligne 95)
- **Signature / Extrait** : `export const getCachedData = async (client, key) => {`
- **Rôle Fonctionnel** : Cette fonction/méthode est responsable de l'exécution d'une étape clé du traitement.
- **Mode d'Exécution** : Asynchrone avec retour de Promesse (`Promise`).

#### 🔹 `invalidateCache` (Ligne 114)
- **Signature / Extrait** : `export const invalidateCache = async (client, pattern) => {`
- **Rôle Fonctionnel** : Cette fonction/méthode est responsable de l'exécution d'une étape clé du traitement.
- **Mode d'Exécution** : Asynchrone avec retour de Promesse (`Promise`).
