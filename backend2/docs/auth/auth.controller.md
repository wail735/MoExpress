# 📚 Documentation Technique Détaillée : `auth.controller.js`

**Emplacement Source** : `auth/auth.controller.js`  
**Portée** : Backend Serveur (Node.js / Express)  
**Nombre de Lignes Code** : 129 lignes  

---

## 1. 📌 Rôle et Responsabilité du Fichier

Le fichier `auth.controller.js` est un composant clé de l'architecture MoExpress (AliExpress Clone).
...
| :--- | :--- | :--- |
| L7 | `import * as authService from "./auth.service.js";` | Module indispensable pour les fonctionnalités du fichier. |
| L10 | `import { registerDTO, loginDTO, forgotPasswordDTO, resetPasswordDTO } from "./auth.dto.js";` | Module indispensable pour les fonctionnalités du fichier. |
...
#### 🔹 `register` (Ligne 15)
- **Signature / Extrait** : `export const register = async (req, res, next) => {`
...
#### 🔹 `login` (Ligne 40)
- **Signature / Extrait** : `export const login = async (req, res, next) => {`
- **Rôle Fonctionnel** : Cette fonction/méthode est responsable de l'exécution d'une étape clé du traitement.
- **Mode d'Exécution** : Asynchrone avec retour de Promesse (`Promise`).

#### 🔹 `forgotPassword` (Ligne 64)
- **Signature / Extrait** : `export const forgotPassword = async (req, res, next) => {`
- **Rôle Fonctionnel** : Cette fonction/méthode est responsable de l'exécution d'une étape clé du traitement.
- **Mode d'Exécution** : Asynchrone avec retour de Promesse (`Promise`).

#### 🔹 `resetPassword` (Ligne 85)
- **Signature / Extrait** : `export const resetPassword = async (req, res, next) => {`
- **Rôle Fonctionnel** : Cette fonction/méthode est responsable de l'exécution d'une étape clé du traitement.
- **Mode d'Exécution** : Asynchrone avec retour de Promesse (`Promise`).

#### 🔹 `getMe` (Ligne 106)
- **Signature / Extrait** : `export const getMe = async (req, res, next) => {`
- **Rôle Fonctionnel** : Cette fonction/méthode est responsable de l'exécution d'une étape clé du traitement.
- **Mode d'Exécution** : Asynchrone avec retour de Promesse (`Promise`).

#### 🔹 `seed` (Ligne 120)
- **Signature / Extrait** : `export const seed = async (req, res) => {`
- **Rôle Fonctionnel** : Cette fonction/méthode est responsable de l'exécution d'une étape clé du traitement.
- **Mode d'Exécution** : Asynchrone avec retour de Promesse (`Promise`).

#### 🔹 `seedDatabase` (Ligne 122)
- **Signature / Extrait** : `const seedDatabase = (await import("../config/seed.js")).default;`
- **Rôle Fonctionnel** : Cette fonction/méthode est responsable de l'exécution d'une étape clé du traitement.
