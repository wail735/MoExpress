# 📚 Documentation Technique Détaillée : `order.controller.js`

**Emplacement Source** : `orders/order.controller.js`  
**Portée** : Backend Serveur (Node.js / Express)  
**Nombre de Lignes Code** : 66 lignes  

---

## 1. 📌 Rôle et Responsabilité du Fichier

Le fichier `order.controller.js` est un composant clé de l'architecture MoExpress (AliExpress Clone).
...
| :--- | :--- | :--- |
| L6 | `import * as orderService from "./order.service.js";` | Module indispensable pour les fonctionnalités du fichier. |
| L7 | `import { createOrderDTO, updateOrderStatusDTO } from "./order.dto.js";` | Module indispensable pour les fonctionnalités du fichier. |
...
#### 🔹 `createOrder` (Ligne 12)
- **Signature / Extrait** : `export const createOrder = async (req, res) => {`
...
#### 🔹 `getMyOrders` (Ligne 31)
- **Signature / Extrait** : `export const getMyOrders = async (req, res) => {`
- **Rôle Fonctionnel** : Cette fonction/méthode est responsable de l'exécution d'une étape clé du traitement.
- **Mode d'Exécution** : Asynchrone avec retour de Promesse (`Promise`).

#### 🔹 `trackOrder` (Ligne 43)
- **Signature / Extrait** : `export const trackOrder = async (req, res) => {`
- **Rôle Fonctionnel** : Cette fonction/méthode est responsable de l'exécution d'une étape clé du traitement.
- **Mode d'Exécution** : Asynchrone avec retour de Promesse (`Promise`).

#### 🔹 `updateOrderStatus` (Ligne 55)
- **Signature / Extrait** : `export const updateOrderStatus = async (req, res) => {`
- **Rôle Fonctionnel** : Cette fonction/méthode est responsable de l'exécution d'une étape clé du traitement.
- **Mode d'Exécution** : Asynchrone avec retour de Promesse (`Promise`).
